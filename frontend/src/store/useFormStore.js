...
export const useForumStore = create((set, get) => ({
  ...
  fetchAllForums: async () => {
    ...
    try {
      ...
      const response = await axiosInstance.get(
        `/forums/all-forums?page=${page}&limit=${limit}`
      );
  ...
  sendMessage: async (forumId, formData) => {
    set({ isLoading: true, error: null });
    try {
      const response = await axiosInstance.post(
        `/forums/send-message/${forumId}`,
        formData,
        {
          headers: {
      ...
      const newMessage =
        response.data.forum.messages[response.data.forum.messages.length - 1];
  ...
  joinForum: async (forumId) => {
    ...
    try {
      ...
      set((state) => ({
        ...
        currentForum:
          state.currentForum?._id === forumId
  ...
  createForum: async (title, description) => {
    set({ isLoading: true, error: null });
    try {
      const response = await axiosInstance.post("/forums/create-forum", {
        title,
        description,
      });
